...
def load_unsloth_model(
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        ...
        max_lora_rank=lora_rank,    # Maximum LoRA rank
        gpu_memory_utilization=gpu_memory_utilization,
        # CRITICAL: Disable CUDA graphs to prevent Triton OOM crashes
        # Unsloth standby mode overrides gpu_memory_utilization anyway
        enforce_eager=True,  # Bypass CUDA graph capture issues
    )
...
def create_grpo_config(output_dir: str, tokenizer, max_steps: int = -1):
    ...
    config = GRPOConfig(
        ...
        num_generations=2,  # Reduced from 4 to save memory for vLLM standby
        temperature=0.6,
