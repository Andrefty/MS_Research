...
EVAL_SCRIPT_PATH="$WORK_DIR/inference_experiment/eval_command.sh"
...
(
    ...
    if [ $ACTIVATION_STATUS -eq 0 ]; then
        ...
        echo "Running eval_command.sh from $EVAL_SCRIPT_PATH..."
        bash "$EVAL_SCRIPT_PATH" 
        SCRIPT_STATUS=$?
        echo "eval_command.sh finished with status: $SCRIPT_STATUS"
