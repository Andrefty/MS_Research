...
echo "SGLang server is up. Proceeding with evaluation script."
EVAL_SCRIPT_PATH="$WORK_DIR/inference_experiment/eval_command_8B.sh"
EVAL_CWD="$WORK_DIR/PrimeVul"

echo "Running evaluation script in $EVAL_CWD with environment $PRIMEVUL_ENV_NAME..."
...
(
    ...
    if [ $ACTIVATION_STATUS -eq 0 ]; then
        ...
        echo "Running eval_command_8B.sh from $EVAL_SCRIPT_PATH..."
        bash "$EVAL_SCRIPT_PATH" 
        SCRIPT_STATUS=$?
        echo "eval_command_8B.sh finished with status: $SCRIPT_STATUS"
...
if [ $EVAL_STATUS -ne 0 ]; then
    echo "Evaluation script execution block finished with errors (status: $EVAL_STATUS)."
else
    echo "Evaluation script execution block finished successfully."
fi
echo "Full evaluation log is available at: $EVAL_LOG_FILE"
echo "Last 20 lines of evaluation log:"
